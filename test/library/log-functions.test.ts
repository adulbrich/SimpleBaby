...
describe("saveLog()", () => {
    ...
    test("catch local-store.ts->getActiveChildId() error", async () => {
        ...
        expect(result.success).toBe(false);
        expect(result.error).toBe("No active child set (guest mode)");
    });
    ...
    test("catch local-store.ts->insertRow() error", async () => {
        ...
        expect(result.success).toBe(false);
        expect(result.error).toBe(`Failed to save ${testLogType} log locally.`);
    });
...
async function imageError(testErrorMessage: string) {
    ...
    expect(result.success).toBe(false);
    expect(result.error).toBe("Photo upload failed");
