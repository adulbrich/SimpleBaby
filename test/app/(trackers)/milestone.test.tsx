...
jest.mock("@/library/utils", () => {
    const getActiveChildData = jest.fn(async () => ({ success: true, childId: "test-child-id" }));
    return {
        getActiveChildData: getActiveChildData,
    };
});
...
describe("Track milestone screen", () => {
    ...
    test("Catch getUser error", async () => {
        ...
        expect((Alert.alert as jest.Mock).mock.calls[0][0]).toBe(`Failed to save milestone log: Photo upload failed`);
