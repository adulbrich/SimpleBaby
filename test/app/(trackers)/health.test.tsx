...
jest.mock("@/library/utils", () => ({
    getActiveChildData: jest.fn(async () => ({ success: true })),  // default case, should cause no immediate error handling
}));
...
describe("Track health screen", () => {
    ...
    test("Catch unfilled vaccine inputs", async () => {
        ...
        expect((Alert.alert as jest.Mock).mock.calls[0][0]).toBe("Missing Information");
        expect((Alert.alert as jest.Mock).mock.calls[0][1])
            .toBe("Failed to save the Vaccine Health log. You are missing the following fields: vaccine name.");
    });
    ...
    test("Catch unfilled other inputs", async () => {
        ...
        expect((Alert.alert as jest.Mock).mock.calls[0][0]).toBe("Missing Information");
        expect((Alert.alert as jest.Mock).mock.calls[0][1])
            .toBe("Failed to save the 'Other' Health log. You are missing the following fields: health event name.");
    });
    ...
    test("Catch getActiveChildData error", async () => {
        ...
        // error message generated by library/utils.ts -> getActiveChildData()
        // Alert.alert() called by app/(trackers)/health.tsx -> handleSaveHealthLog()
