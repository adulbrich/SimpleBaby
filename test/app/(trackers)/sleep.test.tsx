...
describe("Track sleep screen", () => {
    ...
    test("Catch invalid user", async () => {
        ...
        expect((Alert.alert as jest.Mock).mock.calls[0][0]).toBe("Error: No authenticated user found");

        // error message generated by app/library/utils.ts -> getActiveChildId()
        // Alert.alert() called by app/(trackers)/sleep.tsx -> handleSaveSleepLog()
        expect((Alert.alert as jest.Mock).mock.calls[1][0]).toBe("Failed to save sleep log: No authenticated user found");
    });
    ...
    test("Catch no active child", async () => {
        ...
        // error message generated by app/library/utils.ts -> getActiveChildId()
        // Alert.alert() called by app/(trackers)/sleep.tsx -> saveSleepLog()
        expect((Alert.alert as jest.Mock).mock.calls[0][0]).toBe("Error: No active child set in user metadata");

        // error message generated by app/library/utils.ts -> getActiveChildId()
        // Alert.alert() called by app/(trackers)/sleep.tsx -> handleSaveSleepLog()
        expect((Alert.alert as jest.Mock).mock.calls[1][0]).toBe("Failed to save sleep log: No active child set in user metadata");
    });
