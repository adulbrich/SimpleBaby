...
const TEST_LOGS = [{
    ...
    category: "test category 1 U2FsdGVkX1",
    item_name: "test item 1 U2FsdGVkX1",
    ...
    id: "test log id 2",
    child_id: TEST_CHILD_ID,
    category: "test category 2 U2FsdGVkX1",
    item_name: "test item 2 U2FsdGVkX1",
    amount: "test amount 2 U2FsdGVkX1",
    feeding_time: (new Date("1 Jan 2000")).toISOString(),
    note: "",
}];
...
describe("Feeding logs screen", () => {
    ...
    test("Displays delete log confirmation", async () => {
        ...
        for (const log of TEST_LOGS) {
            await userEvent.press(
                screen.getByTestId(`feeding-logs-delete-button-${log.id}`)
            );

            // Alert.alert called by feeding-logs.tsx -> handleDelete()
    ...
    test("Catches delete log error", async () => catchDeleteError(() =>
        ...
        // { error: /* truthy value */ }
        // This should cause error handling in callback defined in app/(logs)/feeding-logs.tsx -> handleDelete
...
async function rendersLogButtons() {
    ...
    for (const log of TEST_LOGS) {
        expect(screen.getByTestId(`feeding-logs-edit-button-${log.id}`)).toBeTruthy();
        expect(screen.getByTestId(`feeding-logs-delete-button-${log.id}`)).toBeTruthy();
    }
}
...
async function catchDecryptionError() {
    ...
    // This should cause error handling in app/(logs)/feeding-logs.tsx -> fetchFeedingLogs() -> safeDecrypt()
...
async function rendersLogs() {
    ...
    for (const log of TEST_LOGS) {
        expect(screen.getByText(format(new Date(log.feeding_time), 'MMM dd, yyyy'), {exact: false})).toBeTruthy();
        expect(screen.getByText(format(new Date(log.feeding_time), 'h:mm a'), {exact: false})).toBeTruthy();
        expect(screen.getByText(await decryptData(log.category), {exact: false})).toBeTruthy();
        expect(screen.getByText(await decryptData(log.item_name), {exact: false})).toBeTruthy();
...
async function catchDeleteError(mockFailingDelete: () => void) {
    ...
    for (const log of TEST_LOGS) {
        ...
        // Alert.alert called by feeding-logs.tsx -> handleDelete() -> anonomous callback
...
async function deletesLog() {
    ...
    for (const log of TEST_LOGS) {
        ...
        expect(() => screen.getByText(format(new Date(log.feeding_time), 'MMM dd, yyyy'), {exact: false})).toThrow();
        expect(() => screen.getByText(format(new Date(log.feeding_time), 'h:mm a'), {exact: false})).toThrow();
        expect(async () => screen.getByText(await decryptData(log.category), {exact: false})).rejects.toThrow();
        expect(async () => screen.getByText(await decryptData(log.item_name), {exact: false})).rejects.toThrow();
...
async function updateDisplayedLogs(mockFetchLogs: (newLogs: object) => void) {
    ...
    const editedLog = {
        ...
        category: "edited category U2FsdGVkX1",
        item_name: "edited item U2FsdGVkX1",
    ...
    expect(screen.getByText(await decryptData(editedLog.item_name), {exact: false})).toBeTruthy();
