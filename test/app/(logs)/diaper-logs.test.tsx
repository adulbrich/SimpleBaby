...
describe("Diaper logs screen", () => {
    ...
    test("Passes current values to edit log pop-up", async () => {
        ...
        for (const log of TEST_LOGS) {
            ...
            // check field values
            expect(editingLog.consistency.value).toBe(await decryptData(log.consistency));
...
async function updateDisplayedLogs(mockFetchLogs: (newLogs: object) => void) {
    ...
    await act(async () => {
        ...
        expect(displayValues.includes(await decryptData(editedLog.consistency))).toBeTruthy();
        ...
        expect(displayValues.includes(await decryptData(log.consistency))).toBeFalsy();
