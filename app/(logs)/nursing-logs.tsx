...
const NursingLogsView: React.FC = () => {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            const updated = {
                ...
                right_duration: editingLog.right_duration ? await encryptData(editingLog.right_duration) : null,
    ...
    const renderNursingLogItem = ({ item }: { item: NursingLog }) => (
        <View className="bg-white rounded-xl p-4 mb-4 shadow">
            <Text className="text-lg font-bold mb-2">{format(new Date(item.logged_at), 'MMM dd, yyyy')}</Text>
            <Text className="text-base mb-1">Time Logged: {format(new Date(item.logged_at), 'h:mm a')}</Text>
            {item.left_duration && <Text className="text-base mb-1">Left Duration: {item.left_duration}</Text>}
            {item.right_duration && <Text className="text-base mb-1">Right Duration: {item.right_duration}</Text>}
            {item.left_amount && <Text className="text-base mb-1">Left Amount: {item.left_amount}</Text>}
            {item.right_amount && <Text className="text-base mb-1">Right Amount: {item.right_amount}</Text>}
            {item.note && <Text className="text-sm italic text-gray-500 mt-1">📝 {item.note}</Text>}
                ...
                >
                    <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 16, backgroundColor: '#00000099' }}>
                        <View className="bg-white w-full rounded-2xl p-6">
                            ...
                            {['left_duration', 'right_duration', 'left_amount', 'right_amount', 'note'].map((field, idx) => (
                                <View key={idx} className="mb-3">
                                    <Text className="text-sm text-gray-500 mb-1 capitalize">{field.replace('_', ' ')}</Text>
                                    <TextInput
                                        className="border border-gray-300 rounded-xl px-3 py-2"
                                        value={editingLog?.[field as keyof NursingLog] || ''}
                                        onChangeText={(text) =>
                                            setEditingLog((prev) => prev ? { ...prev, [field]: text } : prev)
                                        }
                                    />
                                </View>
                            ))}
